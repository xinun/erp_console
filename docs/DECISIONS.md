...
## 2026-08-04: 고객 문의 연결을 회사 Jira로 통합

### 결정

별도의 고객 문의(JSM) 연결과 검색 범위 UI를 제거하고 회사 Jira 검색에 통합한다.

### 이유

현재 고객 문의 기능은 JSM 전용 API가 아니라 Jira 이슈 검색 API를 사용한다. 같은 회사 계정과 사이트를 다시 연결하는 것은 중복이며, JSM 문의도 사용자가 접근 가능한 Jira 이슈 검색 결과에 포함된다.

### 결과

- 사용자는 회사 Jira를 한 번만 연결한다.
- 별도 고객 문의 결과 탭과 집계를 표시하지 않는다.
- 향후 JSM 전용 API 기능이 필요해질 때만 별도 연결을 재검토한다.
